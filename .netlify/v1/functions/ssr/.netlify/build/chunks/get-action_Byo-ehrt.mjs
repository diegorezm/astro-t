import { A as AstroError, s as ActionNotFoundError } from './astro/assets-service_Dh1T9ksQ.mjs';

async function getAction(path) {
  const pathKeys = path.replace("/_actions/", "").split(".").map((key) => decodeURIComponent(key));
  let { server: actionLookup } = await import('./_astro_internal-actions_DPZk9I2F.mjs');
